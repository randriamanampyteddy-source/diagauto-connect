...
exports.getStats = async (req, res) => {
  try {
    ...
    const [[{ total_rdv }]] = await db.query("SELECT COUNT(*) as total_rdv FROM rendezvous WHERE DATE(date_rdv) = CURDATE()");
    ...
    const [[{ chiffre_affaires }]] = await db.query("SELECT COALESCE(SUM(montant_paye), 0) as chiffre_affaires FROM factures WHERE MONTH(date_facture) = MONTH(CURDATE()) AND YEAR(date_facture) = YEAR(CURDATE())");
